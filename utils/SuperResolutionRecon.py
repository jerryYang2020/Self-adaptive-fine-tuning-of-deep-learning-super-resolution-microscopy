...
def SuperResolutionRecon(model, lrImgPath, edgeImgPath, datadir_save, scale, predict_size, step, gap,imagetypeSave='.tif'):
    ...
    FilenameSave = os.path.split(imagedata_dir)[1].split('.')[0]+'_SuperResolution'
    ...
    for i in tqdm(range(imagelen)):
        ...
        input_imagepredict3 = input_imagepredict3[gap * bin:img_size[0] * bin - gap * bin,
                              gap * bin:img_size[1] * bin - gap * bin]
    ...
    output_path = gen_video.merge_image_to_video(filename, output_format='mp4', jpg_folder = jpg_path_name, output_folder=output_folder)
    return output_path
