...
def SRmodel_SSF (train_set,lrImgPath, edgeImgPath, datadir_save, SFSRMmodeldir, DENmodeldir, scale, iteration_times, sigma, ksize, SRlr, continuity_level, sparse_level, consistency_level, joint_Training, predict_size, step, gap):
    ...
    for epoch in tqdm(range(iteration_times)):
        ...
        fineTuningDir = ReconSR(model,bin,predict_size,step,gap,datadir,edgedir,savedir,FilenameSave=str(epoch),imagetypeSave='.tif')
        ...
        for iteration, batch in (enumerate(training_data_loader, 1)):
            ...
            Sparseloss =torch.abs(torch.mean(out)-torch.mean(out_contrast))
            ...
            if len(list(out_squeeze.shape))==3:
                ...
                total_loss = degloss_1 + Hessianloss * continuity_weight + Sparseloss*sparse_level + enhanceloss * consistency_level

            if len(list(out_squeeze.shape))==2:

                total_loss = degloss_1 + Sparseloss*sparse_level + enhanceloss * consistency_level
        ...
        torch.save(model, os.path.join(modelsavedir,str(epoch+1)+'.pth'))
    ...
    dataframe = pd.DataFrame({'DRSE': record[:, 0], 'Hessian': record[:, 1],'Sparse': record[:, 2],
                              'Rotation': record[:, 3],'total': record[:, 4]})
    dataframe.to_csv(os.path.join(savedir, 'SFSRM-SAFT_Finetuning_Record.csv'), index=False, sep=',')

    return model, os.path.join(datadir_save, fineTuningDir)